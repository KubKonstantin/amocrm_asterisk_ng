...
## RTP обмен для автотестов

В репозиторий добавлен утилитарный скрипт `scripts/rtp_exchange.py` для проверки RTP-трафика без внешних зависимостей.

### Быстрый запуск

Терминал 1 (эхо-узел):
```bash
python scripts/rtp_exchange.py --local-port 4000 --remote-port 4002 --echo
```

Терминал 2 (генератор RTP):
```bash
python scripts/rtp_exchange.py --local-port 4002 --remote-port 4000 --duration 10
```

По завершении выводится сводка: `sent`, `received`, `parse_errors`, `last_sequence`.

## Config-driven VoIP autotest (GitLab JUnit)

Для запуска комплексной проверки WebRTC signaling + SIP + RTP добавлен раннер:

```bash
python scripts/voip_autotest.py --config ./config.autotest.yml
```

1. Создайте конфиг на основе примера:
```bash
cp config.autotest.example.yml config.autotest.yml
```
2. Заполните адреса своих WebRTC/SIP/RTP endpoints.
3. Запустите раннер, после чего получите JUnit XML отчет (по умолчанию `./artifacts/voip-autotest.junit.xml`).

Важно: все три проверки выполняются в рамках **одного звонка/сессии** с единым `call_id`:
- `call_id` подставляется в WebRTC signaling (`{{call_id}}`),
- тот же `call_id` используется в SIP `Call-ID`,
- RTP-проверка выполняется в той же test-сессии и также помечается тем же `call_id` в отчете.

Этот формат совместим с GitLab CI (`artifacts:reports:junit`).
...
voip_autotest:
  ...
  artifacts:
    when: always
    reports:
      junit: artifacts/voip-autotest.junit.xml
```
